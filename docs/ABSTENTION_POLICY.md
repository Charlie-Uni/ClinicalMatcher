...
Policy `1.1.0` creates a separate restricted prediction set. It never edits the
...
Version `1.1.0` preserves the `1.0.0` precedence and adds one explicit
source-question exception. `med_decisions=absent, value=false` may remain known
with an empty citation because the frozen question says to answer no unless
inability is evidenced. `med_decisions=present` and every other known result
still require patient-local evidence. This exception is not transferable to a
different question or value.
...
2. `unusable_evidence`: a cited evidence ID is outside the patient-local
   evidence inventory;
3. `missing_evidence`: a citation-required known fact cites no evidence;
...
## Historical frozen validation results

The following artifacts were generated by policy `1.0.0` before the
source-question exception was approved. Their hashes and interpretation remain
historical facts and are not relabeled as `1.1.0`. A future real projection
must use `1.1.0` and produce new owner-only hashes.
