...
## 第二台实例的运行与销毁记录（2026-09-18）

在第二台实例上完成：读取器消融 A/B3/B5/C/D（15 个产物）与 E2 的 qwen3:14b、
qwen3:30b-a3b 各 A/B3（12 个产物），全部 scp 回本机 owner-only 目录并验封。
释放前由我远程执行：停止 Ollama、`rm -rf /Users/leaf`，确认 `/Users` 为空、仓库
目录无 JSON 产物；随后 owner 在控制台释放实例。实例上除公开模型权重与代码外
未留任何数据。下次实例需重走环境脚本与最小传输集。
